...
```mermaid
flowchart LR
    A["'사과' in fruits"] --> B{"fruits 안에<br>'사과'가 있나?"}
    B -->|Yes| C["True"]
    B -->|No| D["False"]
```

<highlight>
in 연산자는 list, tuple에서 O(n), set, dict에서 O(1)의 시간복잡도를 가집니다. 많은 데이터에서 검색이 필요하다면 set이나 dict를 활용하세요.
</highlight>

| 연산자 | 설명 |
|--------|------|
| `in` | 값이 목록 안에 있으면 True |
| `not in` | 값이 목록 안에 없으면 True |
