...
<highlight>
논리연산자는 조건문에서 여러 조건을 결합할 때 핵심적으로 사용됩니다. 단락 평가(Short Circuit Evaluation)를 이해하면 더 효율적인 코드를 작성할 수 있습니다.
</highlight>
...
| 연산자 | 설명 | 예시 |
|--------|------|------|
| `and` | 모두 참일 때 True | `True and True` → True |
...
```mermaid
flowchart LR
    A["A and B"] --> B{"A가 False?"}
    B -->|Yes| C["B 평가 안 함<br>False 반환"]
    B -->|No| D["B 평가"]

    E["A or B"] --> F{"A가 True?"}
    F -->|Yes| G["B 평가 안 함<br>A 반환"]
    F -->|No| H["B 평가"]
```
