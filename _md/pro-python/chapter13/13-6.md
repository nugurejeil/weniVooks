...
flowchart LR
    ...
    B --> C["decorator 함수 실행"]
    C --> D["wrapper 함수 반환"]
    D --> E["wrapper 실행 시<br>전처리 → 원본함수 → 후처리"]
```

<highlight>
데코레이터는 `@` 문법으로 함수를 감싸 기능을 추가하는 고차 함수입니다. 로그인 체크, 시간 측정, 로깅 등에 활용됩니다. `@deco`는 `func = deco(func)`와 동일합니다.
</highlight>
...
위 코드는 함수의 에러나 결과를 로그로 남길 수 있습니다. 이러한 로그는 파일로 관리하여 날짜별로도 관리할 수 있습니다.
