...
<highlight>
람다 함수는 `lambda 매개변수: 표현식` 형태의 익명 함수로, `map`, `filter` 등과 함께 간결한 코드 작성에 활용됩니다. 복잡한 로직에는 일반 함수를 사용하세요.
</highlight>
...
람다 함수는 고차 함수(higher-order function)와 함께 사용하여 코드의 유연성을 높일 수 있습니다. 예를 들어, `map`, `filter`와 같은 내장 함수들과 함께 람다 함수를 사용하면 다음과 같이 좀 더 편리하게 코드를 구현할 수 있습니다:
