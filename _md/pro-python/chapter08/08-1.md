...
<highlight>
조건문은 주어진 조건이 참인지 거짓인지 평가하여 코드 흐름을 분기시킵니다. 파이썬에서 0, 빈 문자열, 빈 리스트, None은 False로, 나머지는 True로 평가됩니다.
</highlight>
...
공백은 기본적으로 ‘False’를 의미합니다. 공백을 제외한 나머지(하나 이상의 문자)는 모두 True로 작동을 하게 됩니다.
...
|------------|--------|------|
| 비교 연산자 | `==`, `!=`, `<`, `<=`, `>`, `>=` | `x > 5` |
| 논리 연산자 | `and`, `or`, `not` | `x > 0 and x < 10` |
| 멤버십 연산자 | `in`, `not in` | `'a' in 'apple'` |
| 식별 연산자 | `is`, `is not` | `x is None` |
