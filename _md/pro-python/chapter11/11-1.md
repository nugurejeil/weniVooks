...
flowchart TB
    subgraph 패키지["패키지 폴더"]
        A["모듈1.py<br>함수, 클래스, 변수"]
        B["모듈2.py<br>함수, 클래스, 변수"]
    end
    C["import 모듈"] --> A
    D["from 패키지 import 모듈"] --> 패키지
```

<highlight>
모듈은 함수, 클래스, 변수를 담은 `.py` 파일이고, 패키지는 여러 모듈을 묶은 폴더입니다. `import`로 모듈을 불러오며, `pip install`로 외부 패키지를 설치합니다.
</highlight>
