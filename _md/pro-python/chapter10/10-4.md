...
<highlight>
매직 메서드(던더 메서드)는 `__이름__` 형태의 특별 메서드로, 연산자나 내장 함수의 동작을 정의합니다. `__init__`(생성자), `__str__`(문자열 표현), `__add__`(덧셈) 등이 있습니다.
</highlight>

파이썬의 클래스에는 기본적으로 내장하고 있는 특별 메서드들이 있습니다. 파이썬에서는 이런 내장하고 있는 특별 메서드들을 쉽게 재정의하여 사용할 수 있게 되어있습니다.

마치 마법처럼 간단하게 수정할 수 있다고 해서 매직 메서드라고 부릅니다. 혹은 더블 언더바(\_\_)를 2개 쓰고 있는 점에서 던더(dunder) 메서드라고도 부릅니다.
