...
```mermaid
flowchart TB
    A["부모 클래스 Animal<br>sound 메서드"] --> B["자식 클래스 Dog"]
    A --> C["자식 클래스 Cat"]
    B --> D["sound 재정의<br>왈왈!"]
    C --> E["sound 재정의<br>냐옹!"]
```
...
<highlight>
메서드 오버라이딩은 자식 클래스에서 부모 클래스의 메서드를 재정의하는 것입니다. `super()`로 부모 메서드를 호출하여 확장할 수 있습니다. Python은 오버로딩(같은 이름, 다른 매개변수)을 지원하지 않습니다.
</highlight>
