...
```mermaid
flowchart TB
    A["클래스 Car"] --> B["클래스 변수<br>모든 인스턴스 공유"]
    A --> C["인스턴스 변수<br>각 인스턴스 고유"]
    D["modelx"] --> C
    E["modely"] --> C
    D --> B
    E --> B
...
<highlight>
클래스 변수는 모든 인스턴스가 공유하는 변수이고, 인스턴스 변수는 각 인스턴스가 고유하게 가지는 변수입니다. self를 통해 접근하면 인스턴스 변수, 클래스명으로 접근하면 클래스 변수입니다.
</highlight>
