...
<highlight>
시퀀스 자료형은 인덱싱, 슬라이싱, 연결(+), 반복(*) 등의 공통 연산을 지원합니다. 이 연산들은 모든 시퀀스 타입에서 동일하게 작동합니다.
</highlight>
...
```mermaid
flowchart LR
    A["1, 2, 3, 4"] -->|패킹| B["(1, 2, 3, 4)"]
    B -->|언패킹| C["a, b, c, d"]
```

## 2.1 패킹 (Packing)

패킹은 여러 개의 데이터를 하나의 변수에 담는 과정을 의미합니다. 튜플 패킹을 통해 살펴보겠습니다.
