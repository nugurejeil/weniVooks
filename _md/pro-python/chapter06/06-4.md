...
<highlight>
딕셔너리는 해시 테이블을 기반으로 구현되어 키를 통한 검색, 추가, 삭제가 평균 O(1)의 시간복잡도로 처리됩니다. JSON 데이터 처리에 자주 사용됩니다.
</highlight>

예를 들면, `{'name': 'licat', 'city': 'Jeju', 'job': 'Developer'}`에서 `'name'`은 키이고, `'licat'`은 그에 해당하는 값입니다. 이 딕셔너리의 길이는 3개의 키-값 쌍으로 이루어져 있어요.

이는 어떨 때 쓰일까요? 하나의 사람이 등록시에 주민번호와 학번, 제품번호 등을 같이 입력하게 된다면 바로바로 필요한 값을 찾을 수 있겠죠? 이렇게 저장하게 되면 엑셀처럼 값을 저장하는 것이 가능하게 됩니다.
