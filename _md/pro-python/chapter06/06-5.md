...
<highlight>
집합은 중복을 자동으로 제거하고, in 연산이 O(1)로 매우 빠릅니다. 합집합, 교집합, 차집합 등 수학적 집합 연산을 지원합니다.
</highlight>

사실 셋은 수학의 집합과 거의 비슷하다고 생각하시면 됩니다. 대표적으로 문서 하나에 있는 단어를 셀 때에 모든 단어를 추출하되 중복이 제거된다면 어떤 단어가 있는지 더 파악하기가 쉽겠죠?
...
| 차집합 | `-` | `difference()` | 첫 집합에만 있는 원소 |
| 대칭차집합 | `^` | `symmetric_difference()` | 공통 원소 제외 |
