...
<highlight>
비동기 프로그래밍은 여러 작업을 동시에 진행하는 방식입니다. `async def`로 코루틴을 정의하고 `await`로 실행을 기다립니다. `asyncio` 모듈의 이벤트 루프가 비동기 작업을 관리합니다.
</highlight>
...
```mermaid
flowchart TB
    subgraph 동기["동기 프로그래밍"]
        A1["작업 1"] --> A2["작업 2"]
        A2 --> A3["작업 3"]
    end
    subgraph 비동기["비동기 프로그래밍"]
        B1["작업 1"]
        B2["작업 2"]
        B3["작업 3"]
        B1 -.-> B4["이벤트 루프"]
...
![](/images/python/chapter14/chapter14-6.png)
...
![](/images/python/chapter14/chapter14-7.png '위니브 월드')

::a[위니브월드 Beta]{class='btn-link' href="https://world.weniv.co.kr/" target="\_blank"}

### 6.1.1 동기 vs 비동기 프로그래밍 개념
...
이벤트 루프를 사용하여 여러 개의 이벤트 루프를 관리할 수 있습니다. 이러한 비동기 프로그래밍은 코드 전체의 성능 향상을 도모할 수 있습니다. 또한 응답성을 개선하여 사용자의 만족도를 올릴 수도 있습니다.

코루틴을 실행시킬 때 하나의 코루틴이 이벤트 루프가 되지는 않습니다. 대신, 이벤트 루프 내에 여러 코루틴을 묶어 비동기 프로그래밍을 할 수 있습니다.
