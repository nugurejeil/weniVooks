...
<highlight>
파이썬에서 파일을 다룰 때는 `open()` 함수로 파일을 열고 `close()`로 닫습니다. 모드는 `r`(읽기), `w`(쓰기), `a`(추가)가 있으며, `with`문을 사용하면 자동으로 파일이 닫힙니다.
</highlight>
...
r+모드는 읽기와 쓰기를 동시에 하는 모드입니다. 파일을 읽을 때 처음부터 읽어나가며, 쓰기를 하면 덮어쓰게 됩니다. 파일이 없으면 에러가 나기 때문에 아래처럼 파일 생성 후 테스트해보도록 하겠습니다.
