...
<highlight>
테스팅은 코드가 정확히 동작하는지 검증하는 과정이며, `unittest`, `pytest` 등을 사용합니다. 디버깅은 `pdb` 모듈이나 `breakpoint()`로 버그를 찾아 수정합니다. TDD는 테스트를 먼저 작성하고 코드를 구현하는 개발 방법론입니다.
</highlight>
...
다음은 에러가 없을 경우 출력하는 화면입니다.

![](/images/python/chapter14/chapter14-8.png)

다음은 에러가 있을 경우 출력하는 화면입니다. 에러가 여러 개 도출되었어도 에러는 1개만 출력합니다.
...
![](/images/python/chapter14/chapter14-9.png)
...
테스트 주도 개발(TDD, Test-Driven Development)은 소프트웨어 개발 방법 중 하나입니다. 테스트를 작성하는 데 시간이 걸리지만, 적어도 테스트를 통과한 코드를 만들었기 때문에 어느 정도의 품질을 보장할 수 있습니다.
...
![](/images/python/chapter14/chapter14-10.png)

## 7.5 코드 품질 개선을 위한 정적 분석
...
- **로직이나 나쁜 냄새(code smells):** 과도한 분기, 루프, 파라미터 등으로 인한 코드의 복잡성을 평가합니다. 복잡한 코드는 버그 발생 위험이 높고, 테스트하기 어렵습니다. '코드의 나쁜 냄새' 검색으로 좀 더 많은 케이스를 확인할 수 있습니다.
...
![](/images/python/chapter14/chapter14-11.png)
