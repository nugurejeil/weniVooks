...
<highlight>
파이썬에서 함수는 일급 객체(first-class object)로, 변수에 할당하거나 인자로 전달하거나 반환할 수 있습니다. `def` 키워드는 함수 객체를 생성하고 이름에 바인딩합니다. `__call__` 메서드를 정의하면 클래스를 함수처럼 호출할 수 있습니다.
</highlight>
...
### 12.4.1 `__call__` 메서드를 이용한 호출 가능 객체
