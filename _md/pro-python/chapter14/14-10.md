...
<highlight>
`random` 모듈은 의사 난수(pseudo-random)를 생성합니다. `randint(a,b)`로 정수, `random()`으로 0~1 실수를 생성하고, `choice()`로 요소 선택, `shuffle()`로 순서 섞기가 가능합니다.
</highlight>
...
### 10.1.1 난수 생성(Random Number Generation)
