...
<highlight>
정규표현식은 문자열 패턴을 표현하는 문법으로, `re` 모듈을 통해 사용합니다. `.`(모든 문자), `[]`(택1), `*`/`+`/`?`(수량자), `^`/`$`(시작/끝), `\d`/`\w`/`\s`(캐릭터 클래스) 등의 메타문자를 활용합니다.
</highlight>
...
## 8.1 정규표현식 sample text

Google에서 정규표현식 연습 사이트를 검색하면 2가지 사이트를 추천받을 수 있습니다. regex101.com과 regexr.com입니다. 이 중 regexr.com에서 실습을 하도록 하겠습니다.

미리 실습 환경을 조성해 두었습니다. 해당 샘플은 아래 사이트에서 확인 가능합니다.
...
![](/images/python/chapter14/chapter14-12.png) ![](/images/python/chapter14/chapter14-13.png)

테스트 문자열이 있는 공간입니다. 회원가입을 하시면 여러분만의 별도 테스트 문자열을 만드실 수 있습니다.

![](/images/python/chapter14/chapter14-14.png)

메뉴에서는 Cheatsheet 등 다양한 팁을 확인할 수 있습니다.

![](/images/python/chapter14/chapter14-15.png)

![](/images/python/chapter14/chapter14-16.png)

어떤 패턴에 매칭이 되는지 상세 설명을 하고 있는 공간입니다.

![](/images/python/chapter14/chapter14-17.png)
...
백슬래시를 사용하여 이미 사용되고 있는 특수 문자를 표현할 때 사용합니다.

:::div{.callout}

**대괄호와 소괄호를 인식하기 위해 대괄호 닫기는 이스케이프 문자를 안 넣어도 에러가 안 나는데 소괄호 닫기에는 이스케이프 문자를 꼭 써야 합니다. (해당 챕터 맨 마지막에 테스트한 코드를 넣어두었습니다.)**
