...
<highlight>
f-string은 Python 3.6에서 도입된 문자열 포매팅 방법으로, `f'{변수}'` 형태로 사용합니다. `:`를 사용해 정렬(`<`, `>`, `^`), 소수점 자릿수(`.2f`), 길이 지정 등 다양한 서식을 적용할 수 있습니다.
</highlight>

# 5. f-string 문법과 서식 지정자
...
### 5.7.3 Python 3.6 이상 - f-string 방식

Python 3.6부터는 f-string이라는 더욱 간결하고 직관적인 형식이 도입되었습니다. 이 방식은 문자열 앞에 `f`를 붙여 사용하며, 중괄호 안에 직접 변수 이름이나 표현식을 넣을 수 있습니다. 주로 작은따옴표 앞에 또는 작은따옴표 3개 앞에 사용합니다.
...
아래 노트북에 여러 테스트 코드를 넣어두었습니다. 다양한 상황에서 사용법을 숙지해보세요.

::a[formatting.ipynb]{class='btn-link' href="/images/python/chapter14/chapter14-formatting.ipynb" download}
