...
<highlight>
파이썬의 bool은 int의 하위 클래스입니다. 따라서 True는 1, False는 0으로 산술 연산에 사용될 수 있습니다.
</highlight>
...
timeline
    title bool 타입의 역사
    초창기 : 0과 1로 참/거짓 표현
    Python 2.4 : bool 타입 도입
    Python 3.x : True/False 키워드화
```

파이썬 초창기에는 논리 자료형이 없었습니다. 그래서 `0`과 `1`로 참과 거짓을 나타내었는데 이는 C언어에서 가져온 것입니다. 파이썬에서는 2.4 버전부터 int의 하위 개념(하위 클래스)로 bool 데이터 타입을 도입하였습니다. 초반에는 단순히 하위 클래스에 `True`와 `False`에 1과 0을 할당한 것이라 True나 False에 값을 할당하는 것도 가능했었습니다. 키워드로 사용하게 된 것은 3.x 버전부터입니다.
