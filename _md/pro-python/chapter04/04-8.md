...
<highlight>
메서드 체이닝은 코드를 간결하게 작성할 수 있지만, 과도하게 사용하면 가독성이 떨어지고 디버깅이 어려워집니다. 한 줄에 79자를 넘지 않도록 주의하세요.
</highlight>

다만 너무 과하게 사용하면 가독성을 해칠 우려가 있으며 체이닝 설계가 제대로 되어 있지 않으면 오류가 날 수 있으니 적절하게 사용해주세요.
...
```mermaid
flowchart LR
    A["'010-1000-2000'"] --> B["split('-')"]
    B --> C["리스트<br>['010', '1000', '2000']"]
    C --> D["replace() 호출"]
    D --> E["오류 발생!"]
```
...
| 반환 타입 | 사용 가능한 메서드 예시 |
|----------|------------------------|
| 문자열(str) | replace, upper, lower, strip |
| 리스트(list) | append, pop, sort, reverse |
