...
`st.data_editor`는 데이터 편집기 위젯을 표시합니다. 데이터 편집기 위젯을 사용하면 데이터 프레임 및 다양한 데이터 구조를 테이블 형태의 UI에서 편집할 수 있습니다. 데이터를 테이블로 시각화하기 때문에 사용자가 익숙한 환경에서 데이터를 편집하고 수정할 수 있는 기능을 제공합니다.

```python
st.data_editor(data, *, width=None, height=None, use_container_width=False,
               hide_index=None, column_order=None, column_config=None,
               num_rows="fixed", disabled=False, key=None, on_change=None,
               args=None, kwargs=None, row_height=None)
```
...
- `column_config`(dict, None, Keyword only) : 딕셔너리 형태로 각 열에 대한 표시 방법을 정의합니다.
  - None(기본값) : 열은 각 열의 기본 데이터 유형에 따라 스타일이 지정됩니다.
  - `column_config`는 열의 제목, 표시 여부, 유형 또는 형식, 너비 또는 형식 등 열이 표시되는 방식을 구성하고 최소/최대 값 또는 단계와 같은 속성을 수정할 수 있습니다. 딕셔너리 형태로 각 키는 열 이름이고 값이 아래 값 중 하나인 딕셔너리로 구성되어야 합니다.
    - None : 열을 표시하지 않습니다.
    - 문자열 : 열의 표시 레이블을 지정된 문자열로 변경합니다.
    - `st.column_config` : 열의 상세 표시 설정합니다.
      ex) 열의 표시 이름을 "달러 값"으로 변경하고 각 셀에 "$" 접두사를 추가하려면 st.column_config.NumberColumn("달러 값", format="$ %d"))을 사용합니다.
  - `column_config`에서 정의된 열 유형 중 한 가지를 사용합니다. 인덱스 열을 구성하려면 열 이름으로 \_index를 사용합니다.
...
- `disabled`(bool, Iterable of str, Keyword only) : 열 편집을 제어합니다.
  - True : 모든 열에 대해 편집이 비활성화됩니다. 비활성화된 위젯은 사용자가 어떠한 조작을 입력할 수 없습니다.
  - False(기본값) : 모든 열에 대해 편집이 활성화됩니다.
  - Iterable : Iterable로 지정된 열만 편집이 비활성화됩니다. ex) disabled=("col1", "col2")
- `key`(str, Keyword only) : 위젯의 고유 키로 사용할 문자열로 요소에 ID를 부여하는데 사용합니다. `key`가 생략된 경우, 위젯의 내용을 기반으로 키가 생성됩니다. `key`는 위젯의 상태를 식별하고 추적하기 때문에 동일한 유형의 여러 위젯은 동일한 키를 공유할 수 없습니다. 만약, 동일 위젯이 동일 `label`을 가지고 버튼을 생성하였을 때, 동일한 키를 공유할 수 없어 DuplicateWidgetID 에러가 발생합니다.
...
![스크린샷 2025-05-27 오전 9.40.04.png](/images/streamlit/chapter02/part07/스크린샷_2025-05-27_오전_9.40.04.png)
...
![스크린샷 2025-05-27 오전 9.40.51.png](/images/streamlit/chapter02/part07/스크린샷_2025-05-27_오전_9.40.51.png)

<br>

**예시 3**

다음 예시는 `height` 변수의 값에 따라 높이가 변하는 표를 보여줍니다.
...
edited_df = st.data_editor(df, height = 200, num_rows="fixed")
edited_df = st.data_editor(df, height = None, num_rows="fixed")
```

![스크린샷 2025-05-27 오전 9.41.31.png](/images/streamlit/chapter02/part07/스크린샷_2025-05-27_오전_9.41.31.png)

<br>

**예시 4**

다음 예시는 `width` 변수의 값에 따라 너비가 변하는 표를 보여줍니다.
...
![스크린샷 2025-05-27 오전 9.42.16.png](/images/streamlit/chapter02/part07/스크린샷_2025-05-27_오전_9.42.16.png)

<br>

**예시 5**

다음 예시는 `hide_index` 변수의 값에 따라 index 표시 또는 미표시를 나타내는 표를 보여줍니다.
...
![스크린샷 2025-05-27 오전 9.43.13.png](/images/streamlit/chapter02/part07/스크린샷_2025-05-27_오전_9.43.13.png)

<br>

**예시 6**

다음 예시는 `column_order` 변수를 통해 열의 위치를 바꿔주는 표를 보여줍니다.
...
edited_df = st.data_editor(df, num_rows="fixed", hide_index=False,
                           column_order=("is_widget","command","rating"))
edited_df = st.data_editor(df, num_rows="fixed", hide_index=False,
                           column_order=None)
```

![스크린샷 2025-05-27 오전 9.52.05.png](/images/streamlit/chapter02/part07/스크린샷_2025-05-27_오전_9.52.05.png)
...
![스크린샷 2025-05-27 오전 9.49.54.png](/images/streamlit/chapter02/part07/스크린샷_2025-05-27_오전_9.49.54.png)
