...
이번 챕터에서 배울 핵심 개념들입니다.

| 개념 | 설명 | 예시 |
| --- | --- | --- |
| 조건문 | 조건에 따라 다른 코드를 실행 | `if`, `elif`, `else` |
| 논리 연산 | 여러 조건을 조합 | `and`, `or`, `not` |
...
<highlight>조건문은 프로그램의 흐름을 제어하는 핵심 구조입니다. 조건이 참(True)일 때만 해당 코드 블록이 실행됩니다.</highlight>
...
논리 연산은 평소에 다루던 연산이 아니기 때문에 생소하고 어렵지만 컴퓨터 공학에서는 필수로 다뤄야 하는 개념이며 자주 사용되는 개념입니다. 따라서 반복하여 자연스럽게 사용할 수 있도록 해주세요.
