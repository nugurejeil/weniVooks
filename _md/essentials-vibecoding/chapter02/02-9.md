...
```mermaid
flowchart TD
    A["Skills"] --> B["SKILL.md<br/>(필수)"]
    A --> C["스크립트<br/>(선택)"]
    A --> D["템플릿<br/>(선택)"]
    A --> E["참조 문서<br/>(선택)"]
    B --> F["Claude가 자동으로<br/>관련 Skills 인식 및 로드"]
```
...
## 1.2 Skills의 작동 원리: 점진적 공개(Progressive Disclosure)
...
:::div{.callout}
Skills는 계속해서 업데이트되고 있는 기능입니다. 최신 정보는 공식 문서를 참고해주세요.
