...
# 4. 최선의 경우: 첫 번째 원소가 target → 1번 비교
# 5. 평균의 경우: 중간쯤에서 발견 → n/2번 비교
# 6. 최악의 경우: 마지막 원소가 target 또는 없음 → n번 비교
```
...
## 41.1 예제 1: 배열에서 최댓값 찾기

**문제**: 배열에서 가장 큰 값을 찾아라.

**입력**: n (1 ≤ n ≤ 1,000,000)

### 41.1.1 풀이 1: O(n log n) - 정렬
...
# 45. 시간복잡도: O(n²)
# 46. n = 10,000 → 약 50,000,000번 연산 → 통과
```
...
# 47. 시간복잡도: O(n)
# 48. n = 10,000 → 약 10,000번 연산 → 훨씬 빠름
```
...
다음 장에서는 시간복잡도를 직접 계산하는 연습을 해보겠습니다.
