...
```python
# 문제: 메모리 128 MB
# 필요한 배열 크기: n = 1,000,000
# int 배열: 4 bytes × 1,000,000 = 4 MB → 가능
# int 2차원 배열: 4 bytes × 1,000,000 × 1,000,000 = 4 TB → 불가능
```
...
# n = 10,000 → 10,000 × 10,000 × 4 bytes = 400 MB → 메모리 초과
...
# 실제 사용하는 셀만 저장 → 메모리 절약
```

## 7.3 입력 크기별 공간복잡도 가이드
...
# 작은 정수는 재사용
# Python은 -5 ~ 256 정수를 캐싱하므로 메모리 절약
a = 100
b = 100
# a와 b는 같은 객체를 참조
```
...
## 8.1 문제: 최장 증가 부분 수열 (LIS)

**입력**: n (1 ≤ n ≤ 1,000)

### 풀이 1: DP - O(n²) 시간, O(n) 공간
...
# 시간복잡도: O(n²)
# 공간복잡도: O(n)
# n = 1,000일 때: 약 1,000,000번 연산, 4 KB 메모리
```

### 풀이 2: 이진 탐색 - O(n log n) 시간, O(n) 공간
...
# 시간복잡도: O(n log n)
# 공간복잡도: O(n)
# n = 1,000일 때: 약 10,000번 연산, 4 KB 메모리
```
