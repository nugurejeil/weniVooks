...
<highlight>JWT(JSON Web Token)는 인증에 필요한 정보를 안전하게 전달하기 위한 토큰입니다.</highlight> 쉽게 얘기하여 로그인한 사용자인지 확인하기 위한 토큰입니다. 놀이동산에서 손목에 차는 팔찌와 비슷한 역할을 합니다. 이러한 토큰이 필요한 이유는 HTTP가 무상태(stateless) 프로토콜이기 때문입니다.
...
| 구성 요소 | 설명 |
| --- | --- |
| Header | 토큰 유형과 해싱 알고리즘 지정 |
| Payload | 클레임(사용자 정보, 유효 기간 등) 포함 |
| Signature | 토큰 유효성 검증을 위한 서명 |
