...
<highlight>실제 애플리케이션에서는 데이터를 영구적으로 저장하고 사용하기 위해 데이터베이스를 사용합니다.</highlight> 앞서 우리는 리스트나 딕셔너리로 데이터를 관리했습니다. 이 챕터에서는 간단하게 사용할 수 있는 SQLite를 SQLAlchemy를 통하여 연동하는 방법을 알아보겠습니다. SQLite는 파일 기반의 경량 데이터베이스이며, SQL이라는 언어를 통해 데이터를 저장하고 조회할 수 있습니다. 아래 서비스를 통해 SQL 언어를 간단하게 실습해볼 수 있습니다.
