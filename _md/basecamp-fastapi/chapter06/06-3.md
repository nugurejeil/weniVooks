...
<highlight>예를 들어, GitHub에 코드를 업로드 한다고 했을 때 가상환경까지 포함해서 업로드하진 않습니다.</highlight> 그렇게 되면 너무나 많은 파일들이 업로드 되기 때문입니다. 뿐만 아니라 서버를 이전할 때에도 마찬가지입니다. 모든 환경을 파일로 전달하는 것은 비효율적입니다. 
