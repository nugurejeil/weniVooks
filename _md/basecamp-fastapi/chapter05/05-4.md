...
<highlight>이번 챕터에서는 이전 챕터에서 만들었던 SPA를 MPA로 변경하여 구현해보겠습니다.</highlight> MPA(Multi Page Application)는 여러 페이지로 구성된 애플리케이션입니다. 각 페이지는 서버로부터 새로운 HTML을 받아와 렌더링하는 방식입니다.

## 1.1 MPA 구조

```mermaid
flowchart LR
    A["blog_list.html"] --> B["blog_detail.html"]
    A --> C["blog_create.html"]
    B --> D["blog_edit.html"]
```
