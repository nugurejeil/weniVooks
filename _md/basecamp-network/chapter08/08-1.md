...
| 구분 | 설명 |
|------|------|
| **Stateless(무상태)** | 이전 요청과 다음 요청이 서로 관계없이 독립적으로 처리됨 |
| **Stateful(상태유지)** | 서버가 클라이언트의 상태 정보를 유지함 |

- 위 케이스에서 클라이언트와 서버가 연결되지 않아, 그전에 처리된 요청과 응답이 통신에 영향을 주지 않고 독립적으로 처리됩니다.
- 최초의 통신에서는 웹 브라우저가 사용자와 상호작용하는 것이 아니라 문서를 읽는 리더기 역할만 했기 때문에 상태를 기억할 필요가 없었습니다.
- 우리가 사용하는 HTTP도 무상태 프로토콜입니다.
- 상태를 유지할 필요가 생겼을 때 클라이언트와 서버 측에 각각 상태를 유지할 수 있는 기술들이 도입되었습니다.
