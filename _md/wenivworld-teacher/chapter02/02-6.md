...
| 개념 | 설명 | 예시 |
| --- | --- | --- |
| 비교 연산 | 두 값을 비교하여 True/False 반환 | `10 > 5` → True |
| 내장 함수 | 파이썬에 기본 제공되는 함수 | `min()`, `max()`, `sum()` |
| 리스트 | 여러 값을 순서대로 저장하는 자료형 | `[10, 20, 30]` |
...
<highlight>리스트는 여러 개의 값을 하나의 변수에 순서대로 저장할 수 있는 자료형입니다. 대괄호 []로 감싸서 표현합니다.</highlight>
