...
| 개념 | 설명 | 예시 |
| --- | --- | --- |
| 딕셔너리 | key-value 쌍으로 데이터 저장 | `{'one': 1, 'two': 2}` |
| get 메서드 | 안전하게 값을 꺼냄 | `d.get('key', 0)` |
| 키 에러 방지 | 없는 키 접근 시 기본값 반환 | `d.get('three', '없음')` |

```mermaid
flowchart TD
    A["딕셔너리 값 접근"] --> B["d 대괄호 key"]
    A --> C["d.get key, 기본값"]
    B --> B1["키가 없으면 KeyError"]
    C --> C1["키가 없으면 기본값 반환"]
    C --> C2["안전한 접근 방식"]
```

<highlight>딕셔너리의 get 메서드는 키가 없을 때 에러 대신 지정한 기본값을 반환하여 안전하게 값을 꺼낼 수 있습니다.</highlight>
