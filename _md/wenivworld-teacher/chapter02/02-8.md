...
| 개념 | 설명 | 예시 |
| --- | --- | --- |
| 리스트 | 인덱싱으로 값 호출 및 변경 | `l[0]`, `l[0] = 100` |
| 반복문 | 코드를 반복 실행 | `for`, `while` |
| 형변환 | 자료형을 변환 | `int()`, `str()`, `float()` |
...
flowchart TD
    ...
    A --> C["while문"]
    B --> B1["순회 가능한 요소에서 반복"]
    C --> C1["조건이 True인 동안 반복"]
```

<highlight>반복문은 같은 코드를 여러 번 실행할 때 사용합니다. for문은 정해진 횟수만큼, while문은 조건이 참인 동안 반복합니다.</highlight>
