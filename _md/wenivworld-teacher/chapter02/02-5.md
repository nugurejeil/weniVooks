...
| 개념 | 설명 | 예시 |
| --- | --- | --- |
| 문자열 | 텍스트 데이터를 저장하는 자료형 | `'hello'` |
| 인덱싱 | 특정 위치의 문자를 가져옴 | `s[0]` → 첫 번째 문자 |
| 슬라이싱 | 문자열의 일부분을 추출 | `s[0:5]` → 0~4번째 문자 |
| 메서드 | 문자열을 조작하는 기능 | `s.replace()`, `s.split()` |
...
<highlight>문자열은 순서가 있는 자료형입니다. 각 문자는 0부터 시작하는 인덱스 번호로 접근할 수 있습니다.</highlight>
