...
**Pandas**는 데이터 분석을 위한 라이브러리로, NumPy와 함께 데이터 분석에 있어서 가장 기본이 되는 라이브러리입니다. Pandas는 데이터를 효과적으로 처리할 수 있는 다양한 기능을 제공합니다.

```mermaid
flowchart LR
    A["Pandas"] --> B["Series - 1차원"]
    A --> C["DataFrame - 2차원"]
    B --> D["인덱스 + 값"]
    C --> E["행 + 열"]
```

이번 챕터에서는 Pandas의 기본적인 사용법을 알아보겠습니다.

## 1.1 Pandas의 데이터 구조

Pandas는 크게 **Series**와 **DataFrame** 두 가지 데이터 구조를 제공합니다.

| 구조 | 차원 | 설명 |
|------|------|------|
| Series | 1차원 | 인덱스가 있는 1차원 배열 |
| DataFrame | 2차원 | 행과 열로 구성된 테이블 형태 |

Series와 DataFrame은 NumPy 배열과 유사하지만, 인덱스를 가지고 있어 데이터를 더 효율적으로 처리할 수 있습니다.
...
- **묵시적 인덱스**: 인덱스를 지정하지 않으면 0부터 시작하는 정수 인덱스가 자동으로 생성됩니다.
- **명시적 인덱스**: 인덱스를 직접 지정하여 데이터를 조회할 수 있습니다.
:::
...
Pandas에서는 `sort_values()`와 `sort_index()` 함수를 사용하여 데이터를 정렬할 수 있습니다.

| 함수 | 설명 |
|------|------|
| `sort_values()` | 특정 컬럼 값을 기준으로 정렬 |
| `sort_index()` | 인덱스를 기준으로 정렬 |

단일 또는 여러 컬럼을 기준으로 오름차순이나 내림차순 정렬이 가능합니다. `ascending` 옵션으로 정렬 방향을 지정할 수 있습니다.
