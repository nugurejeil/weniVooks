...
이번에는 여러 개의 인터페이스를 연결할 때 제네릭을 어떻게 사용하는지 살펴보도록 하겠습니다. 여기서 `U`라는 키워드를 사용했는데 이는 추가적인 타입이 필요할 때 사용하는 것으로 `T`와 같은 역할을 합니다. `S`도 관례상 많이 사용합니다.
...
아래와 같이 여러 개의 제네릭 타입을 사용할 수도 있습니다.
...
이렇게 하면 `add` 함수는 어떤 타입이든 받을 수 있고, 반환값도 받은 타입으로 반환됩니다. 아래 코드는 실제 동작하는 코드입니다.
...
3. (리액트 실전 문제) 제네릭을 사용하여 간단한 상태 관리 Hook을 만들어보세요.

    1. useLocalStorage라는 커스텀 Hook을 만드세요
    2. 이 Hook은 어떤 타입의 데이터든 localStorage에 저장하고 불러올 수 있어야 합니다
    3. 초기값을 설정할 수 있어야 합니다
    4. 값을 변경하면 자동으로 localStorage에 저장되어야 합니다
    ...
    - 나이 입력 (숫자) 
    - 로그인 상태 체크박스 (불린)
    - 페이지 새로고침 후에도 값이 유지되는지 확인

    아래 명령어를 사용하여 리액트 프로젝트를 생성하고 작성해보세요.
    ```shell
    npm create vite@latest my-hook-app -- --template react-ts
    cd my-hook-app  
    npm install
    npm run dev
    ```
...
function App() {
  ...
  return (
    <div className="App">
      <h1>제네릭 useLocalStorage Hook 테스트</h1>
      ...
      <div style={{ margin: '20px 0', padding: '20px', backgroundColor: '#f5f5f5', borderRadius: '8px' }}>
        <h3>현재 저장된 값들:</h3>
        <p><strong>이름:</strong> {name || "(없음)"}</p>
        <p><strong>나이:</strong> {age}</p>
        <p><strong>로그인 상태:</strong> {isLoggedIn ? "로그인됨" : "로그아웃됨"}</p>
      </div>

      <div style={{ margin: '20px 0', padding: '15px', backgroundColor: '#e8f4fd', borderRadius: '8px' }}>
        <p><strong>💡 테스트 방법:</strong></p>
        <p>1. 위의 입력 필드들에 값을 입력해보세요</p>
        <p>2. 페이지를 새로고침해보세요 (F5 또는 Ctrl+R)</p>
        <p>3. 입력한 값들이 그대로 유지되는지 확인해보세요!</p>
      </div>
    </div>
  );
}
