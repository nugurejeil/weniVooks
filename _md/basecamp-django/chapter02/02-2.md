...
date: 2024-07-11
---
<highlight>템플릿 상속은 웹사이트의 공통 디자인을 한 곳에 모아두고 다른 페이지들이 이를 가져다 쓰는 방법이며, 정적(static) 파일은 CSS, JavaScript, 이미지 등 변하지 않는 파일들입니다.</highlight>
...
```mermaid
flowchart TD
    A["사용자 요청"] --> B["config/urls.py"]
    B --> C{"URL 분기"}
    C -->|"''"| D["main/urls.py"]
    C -->|"'blog/'"| E["blog/urls.py"]
    D --> F["main/views.py"]
    E --> G["blog/views.py"]
```
...
```mermaid
flowchart TD
    A["base.html<br>부모 템플릿"] --> B["blog_list.html<br>자식 템플릿"]
    A --> C["blog_detail.html<br>자식 템플릿"]
    A --> D["index.html<br>자식 템플릿"]
