...
<highlight>Django의 View는 사용자의 요청을 받아 처리하고 결과를 반환하는 두뇌 역할을 하며, 함수형 View와 클래스형 View 두 가지 방식으로 작성할 수 있습니다.</highlight>
...
아래는 함수형 View와 클래스형 View의 비교표입니다.

| 특성 | 함수형 View (FBV) | 클래스형 View (CBV) |
|------|------------------|---------------------|
| 코드 형태 | 함수 | 클래스 |
| 직관성 | 높음 | 낮음 (추상화) |
| 재사용성 | 낮음 | 높음 (상속 활용) |
...
```mermaid
flowchart LR
    A["render 함수 호출"] --> B["1 - 템플릿 파일 로드"]
    B --> C["2 - context 데이터 적용"]
    C --> D["3 - 문자열로 변환"]
    D --> E["4 - HttpResponse 반환"]
```
