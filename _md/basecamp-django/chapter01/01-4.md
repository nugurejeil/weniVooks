...
<highlight>Django에서 웹 페이지를 만들려면 앱 생성 → URL 설정 → View 작성 → Template 생성 순서로 진행합니다.</highlight>
...
아래는 Django의 요청-응답 흐름을 나타낸 것입니다.

```mermaid
flowchart LR
    A["사용자<br>브라우저"] -->|"1 - URL 요청"| B["urls.py"]
    B -->|"2 - View 함수 호출"| C["views.py"]
    C -->|"3 - 템플릿 렌더링"| D["template.html"]
    D -->|"4 - HTML 반환"| C
    C -->|"5 - 응답 반환"| A
```
