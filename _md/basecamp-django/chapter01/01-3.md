...
date: 2024-07-04
---
<highlight>Django 프로젝트를 시작하려면 가상환경 생성, Django 설치, 프로젝트 생성 순서로 진행합니다.</highlight>
...
아래는 Django 프로젝트 생성 과정을 요약한 것입니다.

```mermaid
flowchart TD
    A["1 - 프로젝트 폴더 생성"] --> B["2 - 가상환경 생성<br>python -m venv venv"]
    B --> C["3 - 가상환경 활성화"]
    C --> D["4 - Django 설치<br>pip install django"]
    D --> E["5 - 프로젝트 생성<br>django-admin startproject config ."]
    E --> F["6 - DB 마이그레이션<br>python manage.py migrate"]
    F --> G["7 - 서버 실행<br>python manage.py runserver"]
```