...
<highlight>이 프로젝트는 Django를 활용하여 유튜브 클론 서비스를 직접 구현해보는 실습으로, 영상 업로드/재생, 댓글, 태그 검색, 사용자 인증 등의 기능을 포함합니다.</highlight>

# 1. 프로젝트 소개
이 챕터는 구현 화면이 유튜브에 있습니다. 유튜브로 보기 전, 직접 구현해보는 시간을 가져보세요. 여기서는 기획만 제공합니다. 조금 더 실력이 있으신 분들은 AskDjango 이진석님의 유튜브 클론 강의를 시청해주세요. CBV로 구현합니다.

# 2. 기본 요구사항

```mermaid
flowchart TD
    subgraph tube["tube 앱"]
        A1["/tube/"] --> A2["영상 목록"]
        A3["/tube/&lt;pk&gt;/"] --> A4["영상 재생 + 댓글"]
        A5["/tube/create/"] --> A6["영상 업로드<br>🔒 로그인 필요"]
        A7["/tube/update/&lt;pk&gt;/"] --> A8["영상 수정<br>🔒 본인만"]
        A9["/tube/delete/&lt;pk&gt;/"] --> A10["영상 삭제<br>🔒 본인만"]
        A11["/tube/tag/&lt;tag&gt;/"] --> A12["태그 검색"]
        A13["/tube/?q=keyword"] --> A14["키워드 검색"]
    end
    subgraph accounts["accounts 앱"]
        B1["/accounts/signup/"] --> B2["회원가입"]
        B3["/accounts/login/"] --> B4["로그인"]
        B5["/accounts/logout/"] --> B6["로그아웃<br>🔒 로그인 필요"]
        B7["/accounts/profile/"] --> B8["프로필<br>🔒 로그인 필요"]
    end
```
