...
<highlight>Django의 폼(Form)은 사용자 입력 데이터를 안전하게 수집하고 자동으로 유효성을 검사하며, ModelForm을 사용하면 모델과 연동하여 더욱 효율적으로 데이터를 처리할 수 있습니다.</highlight>
...
flowchart LR
    A["사용자 입력"] --> B["form 태그"]
    B --> C{"method?"}
    C -->|"GET"| D["URL 쿼리스트링으로 전송"]
    C -->|"POST"| E["HTTP Body로 전송"]
    D --> F["서버 처리"]
    E --> F
```
