...
<highlight>Django의 settings.py 파일은 프로젝트 전체의 설정을 담당하며, 보안 키, 데이터베이스, 정적 파일, 미들웨어 등 핵심 설정을 관리하고 환경 변수를 통해 안전하게 관리할 수 있습니다.</highlight>
...
```mermaid
flowchart TD
    subgraph settings["settings.py 주요 설정"]
        A["BASE_DIR<br>프로젝트 경로"]
        B["SECRET_KEY<br>보안 키"]
        C["DEBUG<br>디버그 모드"]
        D["INSTALLED_APPS<br>앱 등록"]
        E["MIDDLEWARE<br>미들웨어"]
        F["DATABASES<br>DB 설정"]
        G["TEMPLATES<br>템플릿 설정"]
        H["STATIC/MEDIA<br>정적 파일"]
    end
```
