...
<highlight>Class Based Views(CBV)는 뷰를 클래스로 작성하는 방식으로, ListView, DetailView, CreateView 등의 제네릭 뷰를 상속받아 코드 재사용성을 높이고 CRUD 기능을 쉽게 구현할 수 있습니다.</highlight>
...
```mermaid
flowchart TD
    A["View<br>기본 클래스"] --> B["TemplateView"]
    A --> C["RedirectView"]
    A --> D["ListView"]
    A --> E["DetailView"]
    A --> F["CreateView"]
