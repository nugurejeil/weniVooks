...
<highlight>settings.py를 base, development, production으로 분리하고 python-dotenv로 환경 변수를 관리하면 개발/운영 환경을 효율적으로 분리하고 보안을 강화할 수 있습니다.</highlight>
...
```mermaid
flowchart TD
    A["settings.py<br>(기존)"] --> B["settings/"]
    B --> C["base.py<br>공통 설정"]
    B --> D["development.py<br>개발 환경"]
    B --> E["production.py<br>운영 환경"]
    C --> D
    C --> E
```
