...
date: 2024-07-23
---
<highlight>ORM(Object-Relational Mapping)은 파이썬 객체와 데이터베이스 테이블을 연결해주는 기술로, SQL을 직접 작성하지 않고도 파이썬 문법으로 데이터베이스를 다룰 수 있게 해줍니다.</highlight>
...
```mermaid
flowchart LR
    A["Python 코드<br>Post.objects.all()"] --> B["Django ORM<br>번역기"]
    B --> C["SQL 쿼리<br>SELECT * FROM post"]
    C --> D["데이터베이스"]
    D --> C
    C --> B
    B --> A
...
| Read | `Post.objects.all()`, `get()`, `filter()` | SELECT |
