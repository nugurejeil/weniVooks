...
<highlight>Django의 모델(Model)은 데이터베이스의 구조를 정의하고 데이터를 관리하는 역할을 합니다. 파이썬 클래스로 표현되며, SQL 문법 없이 파이썬 코드로 데이터베이스를 제어할 수 있습니다.</highlight>
...
```mermaid
flowchart LR
    A["models.py 수정"] --> B["makemigrations<br>마이그레이션 파일 생성"]
    B --> C["migrate<br>DB에 적용"]
    C --> D["데이터베이스 변경 완료"]
```
