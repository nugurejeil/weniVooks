...
<highlight>대출(Loan) 기능에서는 LoanForm으로 대출 가능 여부를 검증하고, 대출 생성/목록 조회/반납/연체 확인 뷰를 구현하며, 반납 시 예약자 우선순위를 처리합니다.</highlight>
...
```mermaid
flowchart LR
    A["반납 요청"] --> B["상태 변경<br>RETURNED"]
    B --> C["수량 증가"]
    C --> D{"예약자?"}
    D -->|"있음"| E["예약자에게<br>우선권 부여"]
    D -->|"없음"| F["완료"]
```
