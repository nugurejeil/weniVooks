...
<highlight>Django 사용자 모델은 AbstractUser(권장)와 AbstractBaseUser로 커스터마이징할 수 있으며, AbstractUser는 기본 필드를 제공하고 AbstractBaseUser는 완전한 커스텀이 가능합니다.</highlight>
...
```mermaid
flowchart TD
    A["models.Model"] --> B["AbstractBaseUser<br>최소한의 인증"]
    B --> C["AbstractUser<br>기본 필드 포함"]
    C --> D["User<br>Django 기본 모델"]
```
