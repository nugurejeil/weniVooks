...
::a[샘플 비디오 다운로드 받기]{download class='btn-link' href="/images/html-css/chapter07/rabbit.mp4" target="\_blank"}

<highlight>과거에는 동영상을 플래시 플레이어를 기반으로 개발하였습니다. 이후 플래시를 대체하여 영상을 삽입할 수 있는 video 태그가 탄생한 후, 현재는 비디오 태그를 사용하여 동영상을 재생하는 것이 당연한 것이 되었죠.</highlight> 웹에서 영상을 볼 수 있게 되면서, 유튜브, 넷플릭스와 같은 미디어 플랫폼의 시대로 발전했다고 할 수 있습니다.
...
- video 태그의 속성
  ...
  - `preload`: 페이지를 열 때 어떻게 미리 로드할지를 제어
    ...
    - `metadata` : 비디오 파일을 미리 로딩하지 않지만 파일의 메타데이터를 미리 가져오도록 합니다.
...
- `media` : 화면 크기별로 다른 비디오를 제공할 수 있습니다(반응형)
...
::a[샘플 음원 다운로드 받기]{download class='btn-link' href="/images/html-css/chapter07/CatPurr.mp3" target="\_blank"}
