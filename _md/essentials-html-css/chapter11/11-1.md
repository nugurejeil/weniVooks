...
<highlight>텍스트는 웹 페이지의 핵심 요소입니다. HTML로 텍스트를 작성하고 CSS로 폰트 스타일을 적용할 수 있습니다.</highlight> 이번 장에서는 텍스트를 꾸미는 다양한 CSS 속성들을 살펴보겠습니다.
...
최소한 `4.5:1`의 비율이 되어야합니다. 크롬 개발자 도구에 들어가, 확인하고 싶은 텍스트 color 속성의 색상 선택 도구를 열면 대비율을 확인할 수 있습니다.
...
`@font-face`에 정의되지 않은 `font-weight`를 사용하면 브라우저가 억지로 굵기를 만들어냅니다(fake bold). 품질이 떨어지니 실제 폰트 파일이 있는 weight만 사용하세요!
단, 가변 폰트(Variable Font)는 하나의 파일로 다양한 굵기를 자유롭게 사용할 수 있어요. 대표적인 가변 폰트로는 "Amstelvar" 폰트가 있습니다.

::a[Variable fonts (가변 폰트) | w3schools]{class="btn-link" href="https://wit.nts-corp.com/2019/10/07/5686" target="\_blank"}
...
- 반응형 웹페이지 개발, 유지보수 등에 용이하기 때문에 `em`, `rem`과 같은 상대 단위를 사용합니다.
...
루트요소의 폰트 크기 기본값이 16px로 되어있기 때문에 배수 계산이 어려울 수 있습니다.
:::
