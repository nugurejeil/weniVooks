...
<highlight>`:root`는 문서 트리의 루트 요소를 선택하는 가상 클래스입니다.</highlight> HTML에서는 <html> 요소를 가리키며, 전역 CSS 변수를 선언할 때 주로 사용됩니다. `:root`에서 정의된 변수는 문서 전체에서 사용할 수 있어, 일관된 디자인 시스템을 구축하는 데 핵심적인 역할을 합니다.
...
CSS 변수는 `:root`뿐만 아니라 모든 선택자에서 선언할 수 있습니다. 자식 요소는 부모 요소의 변수를 상속받으며, 같은 변수명이 있다면 더 구체적인 선택자의 값으로 덮어쓰입니다.
