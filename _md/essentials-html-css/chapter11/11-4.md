...
<highlight>CSS에서는 기본 선택자 외에도 특별한 상태나 위치를 지정할 수 있는 가상 클래스와 가상 요소라는 강력한 도구를 제공합니다.</highlight> 이들을 활용하면 HTML 구조를 변경하지 않고도 더욱 세밀하고 동적인 스타일링이 가능해집니다.
...
링크 스타일을 정의할 때는 **L**ink → **V**isited → **H**over → **A**ctive 순서로 써야 합니다. CSS 우선순위 때문에 순서가 바뀌면 제대로 작동하지 않습니다.

**기억법**: "**L**ove **V**egas? **H**ate **A**rizona!"
...
**참고**: `:focus`는 순서와 무관하며 접근성을 위해 항상 포함하는 것이 좋습니다.
...
`::marker`는 리스트 아이템의 마커를 스타일링 할 수 있게 해주는 가상요소입니다. `display: list-item`을 가진 요소에만 적용됩니다. `콘텐츠(content)`, `색상(color)`, `font 관련 속성들` 등의 속성은 사용 가능하지만, 박스 모델의 속성인 `배경(background)`, `테두리(border)`, `안쪽 여백(padding)`은 적용되지 않습니다.
...
**JavaScript vs CSS `:has()` 비교**

`:has()` 선택자가 없었던 시절에는 "이미지가 포함된 카드만 다르게 스타일링"하려면 JavaScript의 도움이 필요했습니다. 어떻게 구현했는지 기존 방식과 새로운 방식을 비교해보겠습니다.

**기존 방식 (JavaScript 필요)**

```javascript
// 이미지 있는 카드 찾아서 클래스 추가
...
```css
.card:has(img) {
  /* 이미지가 있는 카드 스타일 - 바로 적용! */
  padding-top: 0;
}
```

이러한 변화로 코드가 훨씬 간소화되었고, JavaScript에 의존하지 않아도 되므로 런타임 성능이 향상되었습니다. 또한 DOM이 동적으로 변경되어도 CSS가 자동으로 반응하므로 별도의 이벤트 리스너나 업데이트 로직이 불필요합니다.
