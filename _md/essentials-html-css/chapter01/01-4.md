...
<highlight>HTML5라는 용어는 최신 웹 기술을 가리키는 유행어입니다.</highlight> (출처: MDN)

HTML5는 HTML 4.01과의 차별화를 위한 용어로 사용됩니다. 이전 버전인 HTML 4.01에 비해 다양한 새로운 기능과 요소를 포함하고 있어, 더 현대적이고 기능적인 웹 개발을 가능하게 합니다.
...
::a[HTML Living Standard]{class='btn-link' href="https://html.spec.whatwg.org/" target="\_blank"}

## 1.2 Living Standard

이제는 HTML에 버전이 없다고 생각하시면 됩니다. HTML 문서는 다음과 같이 DOCTYPE을 선언하여 Living Standard로 작성된 문서임을 명시합니다.

```html
<!DOCTYPE html>
```

이 선언은 브라우저에게 해당 문서가 HTML로 작성되었음을 알려주는 역할을 합니다. DOCTYPE 선언이 없으면 브라우저가 호환 모드(Quirks Mode)로 동작하여 예상치 못한 렌더링 결과가 발생할 수 있습니다.

:::div{.callout}
DOCTYPE 선언은 항상 HTML 문서의 최상단에 위치해야 합니다.
:::
