...
<highlight>웹은 World Wide Web(WWW)의 줄임말로, 전 세계적으로 분산된 수많은 문서와 기타 리소스들이 서로 연결되어 있는 거대한 정보의 네트워크를 의미합니다.</highlight> 일반적으로 **웹 페이지라고 불리는 문서 형식으로 제공** 되며, **HTML이라는 마크업 언어** 로 작성됩니다. 우리가 흔히 사용하는 웹 브라우저(Chrome, Safari, Firefox, Edge 등)를 통해 이러한 웹 페이지를 열어 볼 수 있습니다.
...
이렇게 HTML, CSS, JavaScript가 웹을 이루는 가장 기초적인 구성 요소입니다. 각각의 역할을 정리하면 다음과 같습니다.

| 언어 | 역할 |
| --- | --- |
| HTML | 웹 콘텐츠의 의미와 구조 정의 |
| CSS | 웹 페이지의 모양과 표현 담당 |
...
<highlight>인터넷은 전 세계의 컴퓨터와 기기들을 서로 연결하는 거대한 네트워크입니다.</highlight> 케이블, 라우터, 서버 같은 물리적 장비들로 구성되어 있으며, 이를 통해 데이터가 한 곳에서 다른 곳으로 전송됩니다. 인터넷 덕분에 우리는 웹사이트 방문, 이메일 주고받기, 파일 공유, 영상 통화 등 다양한 서비스를 이용할 수 있습니다.
...
웹은 인터넷과 다르며, 웹은 인터넷을 기반으로 한 수많은 서비스 중 하나입니다. 간단히 말해서, **인터넷은 인프라**(기반 구조)이며, **웹은 그 위에서 동작하는 서비스** 중 하나입니다.

웹은 **HTTP**(HyperText Transfer Protocol)라는 프로토콜을 사용하여 정보를 전송합니다. 프로토콜은 컴퓨터나 장치 간에 데이터를 주고받는 전송 규칙 또는 약속을 의미합니다. 예를 들어 실생활에서 우리는 택배를 보낼 때, 집 밖에 물건을 놓고 하늘에 "택배 가져가세요!"라고 외치지 않습니다. 택배 회사의 규칙에 따라 택배를 보내게 됩니다. 이때 택배 회사의 규칙이 바로 프로토콜이라고 할 수 있습니다.
...
네트워크는 컴퓨터와 컴퓨터를 연결해주는 망입니다. 작은 네트워크들이 모여 더 큰 네트워크를 형성하고, 이러한 네트워크들이 전 세계적으로 연결된 것이 바로 인터넷입니다.

```mermaid
flowchart LR
    subgraph 네트워크A["네트워크 A"]
        A1["PC"] --- A2["PC"]
    end
    subgraph 네트워크B["네트워크 B"]
        B1["PC"] --- B2["PC"]
    end
    subgraph 네트워크C["네트워크 C"]
        C1["PC"] --- C2["PC"]
    end
    네트워크A --- 네트워크B
    네트워크B --- 네트워크C
...
<highlight>웹은 클라이언트-서버 모델을 기반으로 동작합니다.</highlight> 사용자가 브라우저에 URL을 입력하면, 브라우저는 해당 서버에 요청을 보내고 서버는 응답으로 웹 페이지를 전송합니다.
...
웹 브라우저는 사용자가 입력한 주소(URL)를 웹 서버에 요청하고, 서버는 요청받은 웹 페이지와 필요한 데이터를 조회하여 사용자에게 전달합니다. 이때 사용자가 보는 웹 페이지는 HTML, CSS, JavaScript와 이미지, 비디오, 오디오 등 다양한 리소스들로 이루어져 있습니다.
...
| 구분 | 역할 | 주요 기술 |
| --- | --- | --- |
| 프론트엔드 | 사용자 인터페이스(UI) 개발 | HTML, CSS, JavaScript |
| 백엔드 | 서버와 데이터 처리 | Python, Java, Node.js 등 |
...
- HTML은 _프로그래밍 언어가 아닌_ 콘텐츠의 구조를 정의하는 마크업 언어입니다. 그래서 아래와 같은 다양한 밈이 생성되고 있습니다.
