...
<highlight>`Transition`은 요소의 속성 값이 변할 때, 그 변화가 일정 시간에 걸쳐 점진적으로 일어나도록 하는 기능입니다.</highlight> `transition`은 `transition-property`, `transition-duration`, `transition-timing-function`, `transition-delay`의 단축 속성입니다.
