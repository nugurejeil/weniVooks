...
```jsx
<meta name="description" content="위니브의 다양한 무료 교안을 웹에서 확인해보세요!">
<meta name="author" content="위니브" />
...
짧고 적절한 제목을 사용하는 것이 중요합니다. 제목이 너무 길거나 관련성이 낮으면, 검색 엔진은 검색 결과에 제목의 일부만 표시하거나 자동으로 생성된 다른 제목을 표시할 수 있습니다. 또한, 사용자가 입력한 검색어나 사용 기기에 따라 서로 다른 제목이 표시될 수도 있습니다. 따라서 제목이 너무 길거나 불필요한 내용을 포함하지 않도록 주의해야 합니다. 제목은 약 50-60자를 넘지 않아야 하며, 그보다 길면 브라우저에서 노출되는 제목이 축소되어 보일 수 있습니다.

3. 메타 태그
   메타 태그는 HTML 문서의 `<head>` 섹션에 위치하여 검색 엔진과 브라우저에게 페이지에 대한 중요한 정보를 제공합니다. 사용자에게는 직접적으로 보이지 않지만, SEO와 사용자 경험에 매우 중요한 역할을 합니다.

- `description`: 페이지의 요약본으로, 검색 결과에서 제목 아래에 나타나는 설명 텍스트입니다. 이는 사용자가 검색 결과에서 우리 사이트를 클릭할지 결정하는 중요한 요소입니다.
- 약 150-160자의 길이제한이 있습니다.
- 이 태그는 검색 엔진 결과 페이지에서 웹페이지의 미리보기 텍스트로 사용됩니다
- 웹페이지를 **클릭**하도록 유도하는 광고문구입니다. 중요한 타겟키워드("확인해보세요", "알아보기", "다운로드" 등의 CTA 포함)를 문구에 포함하여 작성하는 것이 좋다고 합니다. 클릭률을 높이고 돋보이게 할 수 있는 방법을 고민해봐야겠죠.
...
오픈 그래프 프로토콜(Open Graph Protocol)은 2010년 Facebook에서 개발한 메타데이터 표준으로, 웹페이지가 소셜 미디어에서 **공유될 때** 어떻게 표시될지를 제어하는 프로토콜입니다.

- 현재 Facebook, Instagram, LinkedIn, Discord, Slack, KakaoTalk 등 대부분의 소셜 플랫폼에서 지원하며, Google도 부분적으로 활용합니다.
