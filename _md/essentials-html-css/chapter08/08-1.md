...
검색엔진은 웹에 존재하는 방대한 정보를 수집·분석하여 사용자의 ‘의도(intent)’에 맞는 결과를 제공하는 시스템입니다. 대표적인 검색엔진에는 **Google, Naver, Daum** 등이 있으며, 최근에는 **ChatGPT, Perplexity** 같은 생성형 AI 기반 검색 플랫폼도 빠르게 확산되고 있습니다.

검색엔진은크롤링(Crawling) → 색인(Indexing) → 순위결정(Ranking)의 과정을 거쳐 웹페이지를 보여줍니다
...
## 3.1 검색 방식의 진화 — Generative Search (생성형 검색)
...
즉, “이 글을 누가 썼는가?”, “이 정보를 제공하는 기관은 신뢰할 수 있는가?”가 검색엔진이 사이트의 품질을 평가하는 중요한 기준이 되었습니다. 이러한 흐름 속에서 **브랜드 신뢰(Brand Trust)** 와 **저자 신뢰(Author Credibility)** 는 검색 노출뿐 아니라 사용자 경험(UX) 전반에 큰 영향을 미치게 되었습니다.
...
```jsx
<meta name="description" content="위니브는 ICT 전문가로 성장하기 위해 필요한 모든 지식과 서비스를 제공하는 ICT 교육 콘텐츠 기업입니다.">
```

HTML 문서의 `<head>`영역에 들어가는 메타태그들은 검색 엔진이 페이지의 주제와 목적을 이해하는 데 큰 도움을 줍니다.

## 4.2 URL 구조 최적화

URL은 사이트의 주소이자, 검색엔진이 구조를 이해하는 중요한 단서입니다.

- 언더바(`_`) 대신 **하이픈() 사용** → Google은 하이픈을 “단어 구분자”로 인식합니다.
  - `lion_resting_on_the_road.png` (❌) `lion-resting-on-the-road.png` (✅)
...
### 4.3.2 `<meta name="description">` 태그
