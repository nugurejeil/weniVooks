...
파이썬이라는 프로그래밍 언어를 설치해야 합니다. 설치하지 않고도 남들이 만들어놓은 MCP SW를 사용할 수도 있지만, 우리가 '자연어'로 원하는 기능을 구현할 수 있는 시대에 안할 이유가 없습니다.

1. 먼저 구글에 **python**을 검색합니다. 파이썬 공식 홈페이지 링크(https://www.python.org) 로 직접 이동하셔도 좋습니다.

![](/images/basecamp-django/chapter01/02-1.png)

2. python 사이트에서 Downloads -> python에서 설치파일을 다운로드합니다. 설치파일을 실행하여, python을 설치하면 됩니다. 12버전 이상을 설치하셔야 정상동작 합니다.
...
Visual Studio Code(VSCode)는 Microsoft에서 개발한 무료 소스 코드 편집기입니다. 메모장을 사용해도 되지만, VSCode에 편의 기능을 사용할 것이기 때문에 설치하도록 하겠습니다. 이러한 에디터가 너무 복잡하다 생각하시는 분들은 메모장을 사용해주세요. 기억하세요. 우리는 '자연어'로 우리가 원하는 기능을 구현할 것입니다.
