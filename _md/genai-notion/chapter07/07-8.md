...
이 장에서는 노션의 핵심 기능인 데이터베이스에 대해 알아봅니다. 데이터베이스를 활용하면 정보를 구조화하고, 다양한 뷰로 시각화하며, 필터와 정렬로 효율적으로 관리할 수 있습니다.
...
<highlight>
데이터베이스 핵심 포인트<br/>
- /데이터로 데이터베이스 생성 (인라인: 현재 페이지, 전체 페이지: 별도 페이지)<br/>
- 갤러리, 보드, 테이블, 리스트, 캘린더, 타임라인, 차트 등 다양한 뷰 지원<br/>
- 필터와 정렬로 원하는 데이터만 조회 가능<br/>
- 수식 속성으로 데이터 간 계산 자동화<br/>
- 관계형과 롤업으로 여러 데이터베이스 간 연결 및 데이터 참조 가능
</highlight>
