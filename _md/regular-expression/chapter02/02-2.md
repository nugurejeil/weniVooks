...
JavaScript에서 정규표현식은 내장 객체로 제공됩니다. 정규표현식을 생성하는 방법은 크게 두 가지가 있습니다.

```mermaid
flowchart TD
    A["JavaScript 정규표현식 생성"] --> B["리터럴 패턴"]
    A --> C["RegExp 생성자"]
    B --> D["/pattern/flags"]
    C --> E["new RegExp 'pattern', 'flags'"]
```

## 1.1 정규표현식 생성 방법

### 1.1.1 리터럴 패턴
리터럴 패턴은 슬래시(`/`)로 감싸서 정규표현식을 생성하는 가장 일반적인 방법입니다. 변수로 할당하지 않고 메서드에서도 바로 사용할 수 있습니다.
...
JavaScript에서 정규표현식을 사용하는 메서드는 크게 두 가지 객체에서 제공됩니다.

| 객체 | 메서드 | 설명 |
|:---:|:---:|:---|
| RegExp | `test()` | 매칭 여부를 boolean으로 반환 |
| RegExp | `exec()` | 매칭 결과를 배열로 반환 |
| String | `match()` | 매칭 결과를 배열로 반환 |
| String | `replace()` | 매칭된 부분을 대체 |
| String | `split()` | 패턴으로 문자열 분할 |
| String | `search()` | 매칭된 위치(인덱스) 반환 |

## 2.1 RegExp 객체 메서드

### 2.1.1 test 메서드
패턴이 문자열과 매칭되는지 확인하여 매칭되는 경우 `true`를 반환합니다. 단순히 매칭 여부만 확인할 때 유용합니다.
...
### 2.2.2 replace 메서드
첫 번째 인자로 전달된 패턴을 두 번째 인자로 전달된 값으로 치환합니다. 캡처 그룹을 사용하면 `$1`, `$2` 등으로 참조할 수 있습니다.
...
<highlight>replace 메서드에서 `$1`, `$2`는 캡처 그룹을 참조합니다. 이를 활용하면 문자열의 순서를 바꾸거나 형식을 변환할 수 있습니다.</highlight>
