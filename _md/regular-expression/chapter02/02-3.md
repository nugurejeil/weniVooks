...
Python에서 정규표현식은 `re` 모듈을 통해 사용할 수 있습니다. `re` 모듈은 Python의 표준 라이브러리이므로 별도의 설치 없이 바로 사용할 수 있습니다.

```mermaid
flowchart TD
    A["Python 정규표현식"] --> B["re.compile으로 패턴 컴파일"]
    A --> C["re 모듈 메서드 직접 사용"]
    B --> D["pattern.match/search/findall 등"]
    C --> E["re.match/re.search/re.findall 등"]
```

## 1.1 re 모듈

`re` 모듈을 사용하는 방법은 크게 2가지로 나뉩니다. 첫 번째는 `re.compile()` 메서드를 사용하여 패턴을 미리 컴파일하고, 두 번째는 `re` 모듈의 메서드를 직접 사용하는 방법입니다.
...
### 1.1.2 raw 문자열 사용
Python에서 raw 문자열을 사용하면 개행 등의 특수문자를 이스케이프하지 않고 그대로 사용할 수 있습니다. 정규표현식에서는 백슬래시(`\`)를 자주 사용하므로 raw 문자열을 사용하는 것이 좋습니다. 아래와 같이 한 번 실행해보세요.
...
| `re.MULTILINE` | `re.M` | 다중 행 모드 - `^`와 `$`가 각 줄에 매칭 |
| `re.DOTALL` | `re.S` | `.`이 개행 문자도 매칭 |
| `re.VERBOSE` | `re.X` | 상세 모드 - 공백과 주석 허용 |
...
Python `re` 모듈의 주요 검색 메서드는 다음과 같습니다.

| 메서드 | 설명 | 반환값 |
|:---:|:---|:---|
| `match()` | 문자열 **시작**부터 매칭 검사 | Match 객체 또는 None |
| `search()` | 문자열 **전체**에서 첫 매칭 검색 | Match 객체 또는 None |
| `findall()` | 모든 매칭을 리스트로 반환 | 리스트 |
| `finditer()` | 모든 매칭을 이터레이터로 반환 | 이터레이터 |

`match`는 문자열의 처음부터 정규식과 매치되는지 조사합니다. 처음부터 매칭이 안되는 경우 `None`을 반환할 수 있기 때문에 문자열 전체를 검색해야 할 때에는 `search`를 사용합니다. `findall`은 정규식과 매치되는 모든 문자열을 리스트로 반환하고, `finditer`는 정규식과 매치되는 모든 문자열을 반복 가능한 객체로 반환합니다.
...
`sub`와 `split` 메서드는 정규식을 사용하여 문자열을 처리할 때 유용합니다.

| 메서드 | 설명 |
|:---:|:---|
| `sub()` | 매칭된 문자열을 다른 문자열로 대체 |
| `split()` | 패턴을 기준으로 문자열 분할 |

`sub`는 문자열의 `replace`와 유사하지만, `replace`는 정규식을 사용할 수 없습니다. 정규식 패턴으로 복잡한 치환을 해야 할 때 `sub`를 사용합니다.
...
각 패턴에 이름을 부여하여 추출할 때 사용할 수 있습니다. 이름을 부여하는 방법은 `(?P<name>...)` 형식으로 사용하면 됩니다. 명명된 그룹을 사용하면 코드의 가독성이 높아지고, 그룹의 순서가 바뀌어도 이름으로 접근할 수 있어 유지보수가 쉬워집니다.

<highlight>명명된 그룹은 `(?P<name>패턴)` 형식으로 작성하고, `group('name')`으로 접근합니다.</highlight>
