...
이전 챕터에서 출제된 두 가지 프로젝트의 해답을 확인해보겠습니다. 먼저 직접 풀어보신 후 해답을 확인하시기 바랍니다.

# 1. 마크다운 파서 솔루션

## 1.1 JavaScript 구현

마크다운 파서는 각 패턴을 순차적으로 적용하여 HTML로 변환합니다. 주의할 점은 **굵게(`**`)를 기울임(`*`)보다 먼저 처리**해야 한다는 것입니다.
...
1. `headers` 패턴에서 `#{1,6}`은 `#`이 1~6개 반복되는 것을 의미
2. `hashes.length`로 `#`의 개수를 세어 헤더 레벨 결정
3. `bold`와 `italic` 처리 순서가 중요 - `**`를 먼저 처리해야 `*`가 잘못 매칭되지 않음
:::
...
로그 분석기는 정규표현식으로 각 로그 라인에서 필요한 정보를 추출한 후, 통계를 계산합니다.

## 2.1 JavaScript 구현

먼저 간단히 한 줄의 로그를 파싱하는 코드부터 살펴보겠습니다.
...
1. **IP 추출**: `(\d+\.\d+\.\d+\.\d+)` - 4개의 숫자 그룹을 점으로 연결
2. **타임스탬프 추출**: `\[(\d+/\w+/\d+:\d+:\d+:\d+)` - 대괄호 안의 날짜/시간 형식
3. **서비스 분류**: 리퍼러 URL에서 도메인을 정규표현식으로 매칭하여 서비스 분류
4. **Counter 활용**: Python의 `collections.Counter`를 사용하면 카운팅이 간편해짐
:::