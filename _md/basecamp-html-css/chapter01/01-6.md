...
Semantic 태그는 HTML 문서를 **의미적으로 구분**하는 태그들로, 문서의 구조와 내용을 더욱 명확하게 표현할 수 있도록 도와줍니다.

```mermaid
flowchart TB
    subgraph Page["웹 페이지 구조"]
        direction TB
        A["header<br>(머리말)"]
        B["nav<br>(네비게이션)"]
        C["main<br>(주요 콘텐츠)"]
...
대표적인 Semantic 태그로는 `<header>`, `<nav>`, `<main>`, `<section>`, `<article>`, `<aside>`, `<footer>` 등이 있습니다. 이 태그들을 적절히 사용하면 **검색 엔진 최적화**(SEO)와 **접근성 향상**에 도움이 됩니다.
...
<highlight>head 태그와 header 태그를 혼동하지 마세요!<br>head는 문서의 메타데이터, header는 문서의 소개 부분입니다.</highlight>

**header 태그 사용 규칙:**
- 중첩해서 사용할 수 없습니다.
- 안에 `<footer>` 태그를 포함할 수 없습니다.
- 각 섹션의 시작 부분에 사용하여 해당 섹션의 소개 부분을 나타낼 수 있습니다.
...
**article vs section 선택 가이드**

| 상황 | 사용할 태그 |
| --- | --- |
| 독립적으로 배포/재사용 가능한 콘텐츠 | `<article>` |
| 앞뒤 문맥 연결이 필요한 콘텐츠 | `<section>` |
| 단순 스타일링 목적 | `<div>` |
