...
```mermaid
flowchart LR
    A["HTML 1.0<br>(1991년)"] --> B["HTML 2.0<br>(1995년)"]
    B --> C["HTML 3.2<br>(1997년)"]
    C --> D["HTML 4.01<br>(1999년)"]
    D --> E["XHTML<br>(2000년)"]
    E --> F["HTML5<br>(2014년)"]
    F --> G["HTML Living Standard<br>(현재)"]
```

HTML의 첫 번째 버전은 1991년에 공개된 HTML 1.0입니다. 그 이후로 HTML은 버전 번호를 붙여가며 업데이트되었으며, 웹 기술의 발전과 함께 여러 차례 개정되었습니다. HTML4.01 버전 이후 HTML은 "Living Standard"라는 이름으로 불리게 되었습니다. 이는 HTML이 더 이상 정적인 버전으로 존재하지 않고, **끊임없이 변화하고 발전하는 살아있는 표준**이라는 의미를 담고 있습니다. 이러한 변화는 웹 기술이 빠르게 발전하는 환경에서 표준이 지속적으로 최신 상태를 유지할 수 있도록 하기 위한 것입니다.
...
HTML의 기본 구조에 대해 알아보겠습니다. 여러분 컴퓨터에서 VS Code를 열어 새로운 HTML 파일을 생성해 보세요.
...
이 파일을 `index.html`이라는 이름으로 저장해 주세요. 저장을 완료했다면 `!` + `Tab`을 눌러 보세요. 기본 템플릿이 생성됩니다.
...
```mermaid
flowchart TB
    subgraph HTML["HTML 문서 구조"]
        direction TB
        A["DOCTYPE"] --> B["html"]
        B --> C["head"]
        B --> D["body"]
        C --> E["meta, title, link 등"]
        D --> F["실제 콘텐츠"]
    end
```
...
이 선언은 **HTML Living Standard 문서**라는 의미입니다. 문서 유형 선언(Document Type Declaration)으로, 현재의 표준을 의미합니다. 이를 통해 브라우저는 해당 문서를 최신 표준에 맞춰 정확하게 해석하고 렌더링할 수 있습니다.

<highlight>DOCTYPE 선언은 반드시 HTML 문서의 최상단에 위치해야 합니다.</highlight>

## 2.2 `<html lang="en">`

`<html>` 태그는 HTML 문서의 **루트(최상단) 요소**입니다. 문서의 모든 내용을 포함하는 최상위 요소로, 문서의 시작과 끝을 나타냅니다.

`lang` 속성을 통해 해당 페이지의 주 언어가 무엇인지 설정할 수 있습니다. 한국어는 `lang="ko"` 또는 `lang="ko-KR"`와 같이 설정할 수 있습니다. 이는 웹 페이지의 언어를 설정하는 것으로, 검색 엔진이나 스크린 리더, 번역 기능 등이 페이지의 언어를 인식하고 사용자에게 적절한 정보를 제공할 수 있도록 합니다.

:::div{.callout}

**VS Code에서 기본 언어를 한국어로 설정하기**

자동완성 시 `en`이 아니라 `ko`로 설정하려면:
1. VS Code에서 `Ctrl` + `,`(콤마) 단축키로 설정 열기
2. `emmet`으로 검색
3. `Emmet: Variables` 찾기
4. `Add Item` 클릭
5. `Item`에 `lang`, `Value`에 `ko` 입력
6. `OK` 버튼 클릭
