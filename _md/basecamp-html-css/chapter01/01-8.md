...
HTML 문서의 구조는 **트리(tree) 형태**로 이루어져 있으며, 각 요소들은 부모-자식, 형제 관계를 가집니다. 이러한 관계를 이해하는 것은 HTML 문서를 효과적으로 작성하고 스타일을 적용하는 데 매우 중요합니다.
...
| 관계 | 설명 | 예시 |
| --- | --- | --- |
| **부모(Parent)** | 다른 요소를 포함하는 요소 | `<section>`은 `<h1>`, `<img>`, `<div>`의 부모 |
| **자식(Child)** | 다른 요소 내부에 직접 포함된 요소 | `<h1>`, `<img>`, `<div>`는 `<section>`의 자식 |
| **형제(Sibling)** | 같은 부모를 가진 요소들 | `<h1>`, `<img>`, `<div>`는 서로 형제 |
| **자손(Descendant)** | 요소 내부의 모든 요소 (자식의 자식 포함) | `<strong>`은 `<section>`의 자손 |
...
형제 요소는 **같은 부모 요소**를 가지는 요소들을 말합니다. 위 예제에서 `<h1>`, `<img>`, `<div>` 태그는 서로 형제 관계에 있습니다.

## 1.4 자손(Descendant) 요소
...
| 선택자 유형 | 문법 | 설명 |
| --- | --- | --- |
| 자손 선택자 | `A B` | A 내부의 모든 B 선택 |
| 자식 선택자 | `A > B` | A의 직접 자식 B만 선택 |
| 인접 형제 선택자 | `A + B` | A 바로 뒤의 형제 B 선택 |
| 일반 형제 선택자 | `A ~ B` | A 뒤의 모든 형제 B 선택 |
