...
CSS 언어에는 충돌 시 어떤 규칙이 우선하는지 규칙이 있습니다. <highlight>이러한 규칙을 계단식(Cascade) 및 우선순위(Specificity)라고 합니다.</highlight>

```mermaid
flowchart TB
    A["CSS 우선순위 원칙"] --> B["1 - 후자 우선의 원칙"]
    A --> C["2 - 구체성의 원칙"]
    A --> D["3 - 중요성의 원칙"]
    B --> E["나중에 작성된 CSS 우선"]
    C --> F["더 구체적인 선택자 우선"]
    D --> G["!important 선언 최우선"]
```
...
| 구체성(명시도)의 원칙 | 더 구체적인 선택자가 우선됩니다 |
| 중요성의 원칙 | `!important` 선언이 최우선됩니다 |
...
| 4 | 요소(타입), 가상 요소 선택자 | 1점 |
| 5 | 전체 선택자 (`*`) | 0점 |

<highlight>위와 순서대로 가중치가 높습니다.</highlight>
