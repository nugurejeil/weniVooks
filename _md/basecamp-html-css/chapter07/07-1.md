...
<highlight>Flexbox는 현대 웹 디자인에서 가장 많이 사용하고 강력한 레이아웃 도구 중 하나입니다.</highlight> 1차원적 레이아웃(행 또는 열)을 쉽게 만들 수 있으며, 요소들의 크기와 순서를 동적으로 조절할 수 있습니다.
...
| 속성 | 적용 대상 | 설명 |
| --- | --- | --- |
| `display: flex` | 컨테이너 | 플렉스 컨테이너 생성 |
| `flex-direction` | 컨테이너 | 주축 방향 설정 |
| `justify-content` | 컨테이너 | 주축 정렬 |
| `align-items` | 컨테이너 | 교차축 정렬 |
| `gap` | 컨테이너 | 아이템 간격 |
| `flex-wrap` | 컨테이너 | 줄 바꿈 설정 |
...
<highlight>**주축을 기준**으로 배열의 위치를 조절하거나 아이템 간의 설정을 할 수 있습니다.</highlight>
...
| `space-evenly` | 시작, 끝, 아이템 사이 모두 동일한 간격 |
...
<highlight>`align-items`는 교차축을 기준으로 정렬합니다.</highlight>
