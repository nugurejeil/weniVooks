...
<highlight>CSS Box Model은 웹 페이지 레이아웃의 근간을 이루는 중요한 개념입니다.</highlight> 모든 HTML 요소는 하나의 '박스'로 취급되며, 이 박스는 콘텐츠, 패딩, 테두리, 마진으로 구성됩니다. Box Model을 이해하면 웹 페이지의 구조를 더 정확하게 제어할 수 있습니다.
...
| 구성 요소 | 설명 |
| --- | --- |
| **콘텐츠(content)** | 텍스트, 이미지 등 실제 보여줄 내용 |
| **패딩(padding)** | 콘텐츠와 테두리 사이의 내부 여백 |
| **테두리(border)** | 요소와 패딩을 감싸는 테두리 |
| **마진(margin)** | 테두리 밖의 외부 여백 |
...
<highlight>`box-sizing` 속성은 요소의 총 너비와 높이를 계산하는 방식을 지정합니다.</highlight> 기본값은 `content-box`입니다. `border-box`로 설정하면 테두리와 패딩을 포함하여 요소의 총 너비와 높이를 계산합니다.

| 값 | 설명 | 계산 방식 |
| --- | --- | --- |
| `content-box` | 기본값 | width/height = 콘텐츠만 |
| `border-box` | border, padding 포함 | width/height = 콘텐츠 + padding + border |
