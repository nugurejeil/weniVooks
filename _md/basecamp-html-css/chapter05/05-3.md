...
웹 개발을 시작하면서 많은 초급 개발자들이 "저는 아직 CSS를 적용하지 않았는데요? 왜 스타일이 자동으로 지정되어 있죠?"라는 의문을 가집니다. 이는 각 브라우저가 HTML 요소에 대해 기본적으로 제공하는 스타일 때문입니다. <highlight>이러한 기본 스타일은 브라우저마다 조금씩 다르며, 이로 인해 동일한 HTML이 브라우저별로 다르게 렌더링 될 수 있기 때문에 일관된 디자인을 구현하기 위해 **reset.css** 파일을 사용합니다.</highlight>

```mermaid
flowchart LR
    A["브라우저 기본 스타일"] --> B["브라우저마다 다름"]
    B --> C["일관성 문제"]
    C --> D["CSS 초기화 필요"]
    D --> E["reset.css / normalize.css"]
```
...
| CSS 초기화 방식 | 특징 | 접근 방식 |
| --- | --- | --- |
| **Reset CSS** | 모든 스타일을 0으로 초기화 | 완전한 초기화 |
| **Normalize.css** | 유용한 기본 스타일 유지 | 일관성 중심 |
| **sanitize.css** | Normalize 기반 + 현대적 트렌드 | 확장형 |
| **CSS Remedy** | 최신 브라우저 대상 | 현대적 접근 |
