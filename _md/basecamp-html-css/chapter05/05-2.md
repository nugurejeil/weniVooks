...
<highlight>`display` 속성은 CSS에서 레이아웃을 결정짓는 중요한 속성 중 하나입니다.</highlight> 이 속성은 요소가 문서 흐름에서 다른 박스들과 어떤 방식으로 배치할지를 결정합니다.

```mermaid
flowchart TB
    A["display 속성"] --> B["block"]
    A --> C["inline"]
    A --> D["inline-block"]
    A --> E["flex"]
    A --> F["grid"]
    A --> G["none"]
    B --> H["줄 바꿈 O, width/height O"]
    C --> I["줄 바꿈 X, width/height X"]
    D --> J["줄 바꿈 X, width/height O"]
```

| 값 | 줄 바꿈 | width/height | margin 상하 | 설명 |
| --- | --- | --- | --- | --- |
| `block` | O | O | O | 한 줄 전체 차지 |
| `inline` | X | X | X | 콘텐츠 크기만큼만 |
| `inline-block` | X | O | O | inline + block 특성 |
| `flex` | - | - | - | 1차원 레이아웃 |
| `grid` | - | - | - | 2차원 레이아웃 |
| `none` | - | - | - | 요소 숨김 |
