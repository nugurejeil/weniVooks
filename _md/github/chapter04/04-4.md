...
프로젝트가 거대해지면, 프로젝트 내의 파일 전체를 `pull` 하는 것보다 특정 폴더만 `clone` 해서 작업하는 것이 더 효율적일 때가 있을 거예요. 이렇게 특정 하위 폴더만 클론 해오는 방법을 알아봅시다.

우선, 클론 받을 새로운 폴더를 생성하고 해당 폴더로 이동합니다. 클론 받을 폴더에 이미 git remote가 연결되어 있다면 `$ git init` 명령어를 입력하세요.

1. `Sparse Checkout` 기능을 활성화합니다.
...
이제 새로운 레파지토리 안으로 이전 레파지토리 히스토리까지 그대로 옮겨집니다!

## 1.3. Repository 병합하기

레파지토리를 관리하다 보면 연관된 여러 개의 레파지토리를 하나의 레파지토리로 병합하고 싶은 순간이 있습니다. 이때, 단순히 새로운 레파지토리에 git push를 하고 기존의 레파지토리를 삭제해 버리면 우리가 열심히 관리한 잔디가 모두 사라지는 불상사가 발생합니다. 위의 레포지토리 이동과 마찬가지로 잔디(commit log)를 그대로 유지하여 레파지토리를 병합하는 방법을 알아볼게요! 병합하기 전에 파일의 충돌이 일어나지 않도록 파일들을 잘 구성해 준 뒤 실행해 주세요.
...
3. 이때 충돌하는 파일이 발생하면 충돌한 파일을 비교해 가며 수정해 주고, 병합에서 제외할 파일이 생긴다면 `git reset HEAD <파일명>` 명령어로 제외합니다.

4. 이제 서로 연관이 없는 커밋 기록들을 병합한 뒤, 병합이 완료된 `myproject01`는 삭제합니다.
