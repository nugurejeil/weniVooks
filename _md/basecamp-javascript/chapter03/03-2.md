...
위의 예시에서 `greet` 함수는 인사 메시지를 출력한 후, 전달받은 함수를 호출합니다. 이렇게 함수 안에서 실행하는 함수나 함수 안에서 반환되는 함수를 콜백 함수라고 합니다. 왜 콜백 함수라고 부를까요? 콜(call)을 받아서 다시 불러주는(back) 함수이기 때문입니다.

```mermaid
sequenceDiagram
    participant Main as 메인 코드
    participant Greet as greet 함수
    participant Callback as sayGoodbye 콜백

    Main->>Greet: greet("licat", sayGoodbye) 호출
    Greet->>Greet: "안녕하세요, licat님!" 출력
    Greet->>Callback: callback() 호출
    Callback->>Callback: "다음에 또 만나요!" 출력
    Callback-->>Greet: 완료
    Greet-->>Main: 완료
```
