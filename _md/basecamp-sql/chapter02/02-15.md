...
<highlight>DELETE 문은 테이블에서 특정 행(Row)을 삭제할 때 사용하는 SQL 구문입니다.</highlight> 조건에 맞는 데이터를 데이터베이스에서 영구적으로 제거할 수 있습니다.

```mermaid
graph LR
    A["1 - DELETE 문 실행"] --> B["2 - WHERE 조건으로<br/>대상 행 선택"]
    B --> C{"3 - 조건 확인"}
    C -->|"일치"| D["4 - 행 삭제"]
    C -->|"불일치"| E["유지"]
```
...
이 쿼리는 `상품ID`가 1인 상품을 삭제합니다.

<highlight>경고: WHERE 절을 사용하지 않으면 테이블의 모든 행이 삭제되므로 매우 주의해야 합니다.</highlight>
...
<highlight>DELETE 문을 사용할 때는 항상 삭제할 데이터를 신중하게 선택하고, 특히 WHERE 절이 빠졌을 경우 테이블 전체 데이터가 삭제될 수 있으므로 주의해야 합니다.</highlight>