...
위 구문에서 `SELECT`는 데이터를 조회할 때 사용하는 키워드입니다. `column1`, `column2`는 조회할 열(Column)의 이름을 나타내며, `table_name`은 조회할 테이블(Table)의 이름을 나타냅니다. <highlight>마지막에는 세미콜론을 사용하여 SQL 구문을 종료합니다.</highlight>
