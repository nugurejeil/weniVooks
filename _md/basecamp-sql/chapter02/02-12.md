...
<highlight>WHERE 절은 데이터베이스에서 특정 조건에 맞는 데이터만 필터링할 때 사용하는 핵심 구문입니다.</highlight> 앞서 배운 비교 연산자, 논리 연산자, `LIKE`, `BETWEEN`, `IN` 등을 모두 WHERE 절과 함께 사용하여 원하는 데이터를 정확하게 조회할 수 있습니다.

WHERE 절은 `SELECT`, `UPDATE`, `DELETE`와 같은 다양한 쿼리에서 사용되지만, 이 장에서는 `SELECT`문을 중심으로 학습합니다.

```mermaid
graph LR
    A["1 - 전체 데이터"] --> B["2 - WHERE 조건 적용"]
    B --> C{"3 - 조건 평가"}
    C -->|"참"| D["4 - 데이터 선택"]
    C -->|"거짓"| E["제외"]
    D --> F["5 - 결과 반환"]
...
<highlight>WHERE 절에서 여러 조건을 결합하려면 AND, OR, NOT 같은 논리 연산자를 사용합니다.</highlight> 이를 통해 복잡한 검색 조건을 만들 수 있습니다.

다음 예시는 카테고리ID가 9**이면서** 가격이 50,000원 이하인 상품을 조회합니다.
