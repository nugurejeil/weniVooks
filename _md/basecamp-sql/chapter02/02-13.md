...
<highlight>INSERT 문은 테이블에 새로운 데이터를 추가(삽입)할 때 사용하는 SQL 구문입니다.</highlight> 데이터베이스에 새로운 레코드를 생성하여 정보를 저장할 수 있습니다.
...
<highlight>중요: 데이터 확인 코드는 이후 실습에서 생략합니다. Ctrl + Z를 눌러 이전 상태로 되돌려 데이터를 확인하세요.</highlight>
