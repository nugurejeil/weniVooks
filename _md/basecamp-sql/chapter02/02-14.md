...
<highlight>UPDATE 문은 기존 테이블의 데이터를 수정(변경)할 때 사용하는 SQL 구문입니다.</highlight> 특정 조건에 맞는 데이터의 값을 새로운 값으로 갱신할 수 있습니다.

```mermaid
graph LR
    A["1 - UPDATE 문 실행"] --> B["2 - WHERE 조건으로<br/>대상 행 선택"]
    B --> C["3 - SET으로<br/>값 변경"]
    C --> D["4 - 데이터 갱신"]
```
...
이 쿼리는 `상품ID`가 1인 상품의 가격을 1,700,000원으로 업데이트합니다.

<highlight>주의: WHERE 절을 사용하지 않으면 테이블의 모든 행이 수정되므로 반드시 조건을 명시해야 합니다.</highlight>
...
<highlight>UPDATE 문을 사용할 때는 항상 WHERE 절을 신중히 사용해야 하며, 조건을 명확히 지정하지 않으면 모든 데이터를 수정할 위험이 있습니다.</highlight>