...
```mermaid
flowchart TB
    subgraph Primitive["원시 타입"]
        PV["변수"] --> PD["실제 값"]
    end
    subgraph Object["객체 타입"]
        OV["변수"] --> OR["참조 주소"] --> OD["실제 데이터"]
    end
```
...
원시타입은 불변성을 가지고 있습니다. 한번 생성된 원시 타입의 값은 변경이 불가능합니다. 원시 값이 할당된 변수에 새로운 값을 할당하면 새로운 값을 메모리에 생성하고 변수가 해당 값을 가리키게 됩니다.

```mermaid
flowchart LR
    subgraph Immutable["원시 타입 - 불변성"]
        I1["값 변경 불가"]
        I2["새 값 할당 시 새 메모리 생성"]
    end
    subgraph Mutable["객체 타입 - 가변성"]
        M1["속성 추가/수정/삭제 가능"]
        M2["같은 메모리 위치에서 변경"]
    end
```
