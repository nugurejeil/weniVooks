...
```mermaid
flowchart TB
    subgraph Global["전역 스코프"]
        G["전역 변수"]
        subgraph Function["함수 스코프"]
            F["함수 내 변수"]
            subgraph Block["블록 스코프"]
                B["블록 내 변수"]
            end
        end
    end
...
:::figure
::img{width="480" src="/images/essentials-javascript/chapter02/01-1.png"}
...
```mermaid
flowchart LR
    subgraph Before["작성한 코드"]
        A1["console.log(name)"]
        A2["var name = 'weniv'"]
    end
    subgraph After["실행 시 해석"]
        B1["var name"]
        B2["console.log(name)"]
        B3["name = 'weniv'"]
    end
