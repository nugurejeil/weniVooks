...
```mermaid
flowchart TB
    subgraph Capturing["캡처링 단계"]
        direction TB
        W1["window"] --> D1["document"] --> B1["body"] --> T1["target"]
    end
    subgraph Bubbling["버블링 단계"]
        direction TB
        T2["target"] --> B2["body"] --> D2["document"] --> W2["window"]
    end
    Capturing --> Bubbling
```
...
화살표 함수에서 `this`는 이벤트 리스너가 등록된 요소를 참조하지 않는다는 것을
주의하세요. 화살표 함수는 자신만의 `this`를 가지지 않고, 화살표 함수를 둘러싸고
