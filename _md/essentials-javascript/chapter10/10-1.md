...
flowchart LR
    subgraph Traditional["기존 방식"]
        T1["요청"] --> T2["전체 페이지 새로고침"]
    end
    subgraph AJAX["AJAX 방식"]
        A1["요청"] --> A2["필요한 데이터만 수신"]
        A2 --> A3["부분 업데이트"]
    end
```
...
ActiveXObject의 등장으로 브라우저는 이전처럼 페이지 전체를 새로고침하지 않고도 HTTP 요청을 보내 서버와 데이터를 주고받고 화면을 갱신할 수 있게 되었습니다. 이는 웹 애플리케이션 개발에 큰 변화를 가져왔습니다. 대표적으로 한 번에 수백명에게 이메일을 보낼 수 있는 최초의 서비스인 Gmail, 실시간으로 지도를 보여주는 구글맵 등이 등장하게 되었습니다.
