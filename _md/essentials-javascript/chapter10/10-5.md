...
```mermaid
flowchart TB
    Start["async 함수 호출"] --> Await1["await - 첫 번째 Promise"]
    Await1 -->|"일시 중지"| Wait1["Promise 처리 대기"]
    Wait1 -->|"처리 완료"| Await2["await - 두 번째 Promise"]
    Await2 -->|"일시 중지"| Wait2["Promise 처리 대기"]
    Wait2 -->|"처리 완료"| End["함수 완료"]
```

async/await를 사용하면 비동기 코드를 더 쉽게 작성할 수 있습니다. async 키워드는 어떤 함수든 프로미스 객체를 반환하게 만들 수 있습니다. await는 async 함수 안에서 사용되며, Promise가 처리될 때가지 함수의 실행을 일시 중지합니다. 처리가 완료되면 프로미스 객체의 fulfilled 값을 반환합니다.
