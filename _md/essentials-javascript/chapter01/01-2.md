...
2. **`async`, `defer` 속성 사용**
   ...
   ```mermaid
   sequenceDiagram
       participant HTML as HTML 파싱
       participant Script as 스크립트

       Note over HTML,Script: 일반 script
       HTML->>Script: 파싱 중단
       Script->>Script: 다운로드 및 실행
       Script->>HTML: 파싱 재개

       Note over HTML,Script: async
       HTML->>HTML: 파싱 계속
       Script->>Script: 병렬 다운로드
       Script->>HTML: 다운로드 완료시 파싱 중단 후 실행

       Note over HTML,Script: defer
       HTML->>HTML: 파싱 계속
       Script->>Script: 병렬 다운로드
       HTML->>Script: 파싱 완료 후 순서대로 실행
   ```
