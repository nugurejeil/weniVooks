...
```mermaid
flowchart TB
    subgraph ThisBinding["this 바인딩 규칙"]
        Global["전역 호출"]
        Method["메서드 호출"]
        Constructor["생성자 호출"]
        Arrow["화살표 함수"]
    end
    Global -->|"this ="| Window["window 객체"]
    Method -->|"this ="| Obj["호출한 객체"]
    Constructor -->|"this ="| Instance["생성된 인스턴스"]
    Arrow -->|"this ="| Lexical["상위 스코프의 this"]
```
...
:::div{.callout}
아래 코드에서 b함수를 누가 호출하나요? 호출하는 객체가 없으므로 일반 함수로 취급됩니다. 따라서 메서드 내부의 함수에서 `this`는 전역 객체를 가리킵니다.
...
화살표 함수에서 `this`는 함수가 정의된 위치의 컨텍스트를 유지합니다. 이를 **lexical this**라고 합니다. 표현이 어렵죠? 화살표 함수의 this는 함수가 정의된 곳의 부모 스코프가 가르키는 this를 상속합니다. 즉, 상위 스코프의 this 와 동일합니다.
...
```
:::div{.callout}
위의 코드에서 이벤트핸들러 함수를 화살표 함수로 변경하면 `this`는 무엇을 참조할까요?
:::
