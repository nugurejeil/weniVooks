...
```mermaid
flowchart LR
    subgraph Nullish["널리시 연산자 ??"]
        N1["null"] -->|"기본값 반환"| Default1["기본값"]
        N2["undefined"] -->|"기본값 반환"| Default1
        N3["0, '', false"] -->|"그대로 반환"| Value["해당 값"]
    end
    subgraph Optional["옵셔널 체이닝 ?."]
        O1["obj?.prop"]
        O2["존재하면 값 반환"]
        O3["없으면 undefined"]
    end
...
```javascript
const zipCode = user?.address?.zipCode || 'zipCode가 존재하지 않습니다.';
