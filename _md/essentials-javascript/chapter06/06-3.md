...
```mermaid
flowchart TB
    FunctionTypes["함수 선언 방식"]
    FunctionTypes --> Declaration["함수 선언식"]
    FunctionTypes --> Expression["함수 표현식"]
    FunctionTypes --> Arrow["화살표 함수"]

    Declaration --> D1["function 함수명() {}"]
    Expression --> E1["const 변수 = function() {}"]
    Arrow --> A1["const 변수 = () => {}"]
```
