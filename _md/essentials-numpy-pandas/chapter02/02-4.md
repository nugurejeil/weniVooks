...
NumPy는 중앙값(**np.median()**), 최빈값 계산을 위한 **np.bincount()**와 **np.argmax()** 등의 함수를 제공합니다. 중앙값은 숫자들을 순서대로 나열했을 때 가운데 있는 값이고, 최빈값은 가장 자주 나타나는 값입니다. 이 함수들을 사용하면 데이터의 분포와 중심 경향을 파악할 수 있습니다.
